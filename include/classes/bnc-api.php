...
define( 'BNC_API_VERSION', '1.1' );

//! The URL used to access the BNC API
define( 'BNC_API_URL', 'http://api.juicegraphic.com/v/' . BNC_API_VERSION );

//! The WordPress setting name for the token information 
define( 'BNC_TOKEN_INFO_SETTING', 'bncid_token' );

define( 'BNC_AUTHENTICATION_ATTEMPTS', 2 );

define( 'BNC_API_TIMEOUT', 8 );
...
class BNCAPI {
	 ...
	 *		http://api.juicegraphic.com/api/v/1.1/user/get_info/
	 *
	...
	function do_api_request( $method, $command, $params = array(), $do_auth = false ) {
        ...
        $options['headers'] = array(
            ...
            'User-Agent' => 'WordPress/' . get_bloginfo("version") . '/WPmob-Pro',
        ...
        if ( !is_wp_error( $raw_response ) ) {
        	if ( $raw_response['response']['code'] == 200 ) {
        		...
        		WPMOB_DEBUG( DEBUG_WARNING, "Unable to connect to server. Response code is " . $raw_response['response']['code'] );
        	}
        } 
	...
	function get_support_posts( $limit = 6, $beta = false ) {
		if ( $this->internal_check_token() ) {
	...
	function authenticate() {
		WPMOB_DEBUG( WPMOB_ERROR, "Authenticating" );
		...
		if ( $nonce ) {
			...
			$auth_data['n'] = $nonce;
			$auth_data['u'] = str_replace( 'http://', '', strtolower( get_bloginfo( 'home' ) ) );
	...
	function check_token() {
		...
		if ( $this->token ) {
			...
			WPMOB_DEBUG( WPMOB_ERROR, "Performing server token check" );
	...
	function internal_check_token() {
		...
		if ( time() > ( $this->token_expires - 15 ) ) {
			WPMOB_DEBUG( WPMOB_INFO, "Authenticating due to expiry" );
		...
		if ( $succeeded && !$this->token_checked ) {	
			...
			if ( ( time() - $this->last_token_check ) > 300 ) {	
				WPMOB_DEBUG( WPMOB_INFO, 'Forcing token check due to cache expiry' );
