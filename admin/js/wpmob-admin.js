...
function WPmobliteAdminReady() {	
	...
	// Make the theme browser and WPmobboard boxes the same height
	jQuery( '#setting-installed-themes .wpmob-theme-box' ).equalHeights( 250 , 500 );
	jQuery( '#touchboard .box-holder' ).equalHeights( 275 , 500 );
	
//	//Make the icon pool float down the page when page is scrolled
//     var offset = jQuery( '#wpmob-icon-menu' ).offset();
//     var topPadding = 15;
//	jQuery( window ).scroll( function() {
//		if (jQuery( window ).scrollTop() > offset.top ) {
//			jQuery( '#wpmob-icon-menu' ).stop().animate( {
//			marginTop: jQuery( window ).scrollTop() - offset.top + topPadding
//		});
//		} else {
//			jQuery( '#wpmob-icon-menu' ).stop().animate( { marginTop: 0 });
//		};
//	});

	// Add handlers for unlicensed pane 5 targets
	...
	// Add the reset confirm to the reset submit button (now that ajax saving has been disabled)	
	...
	jQuery( '#bnc-form' ).click( function() {			
			var totalItems = '';
					...
					function() { 
						...
						jQuery( 'a.delete-set' ).unbind( 'click' ).click( function() {
							...
							if ( confirm( WPmobCustom.are_you_sure_set ) ) {
								...
								WPmobAdminAjax( 'delete-icon-pack', ajaxParams, function( result ) {								
									jQuery('#bnc-form').submit();	
								});
							}
							return false;
		...
		new AjaxUpload( 'manage-upload-button', {
			...
			onComplete: function( file, response ) {
				if ( response == 'invalid' ) {
					...
					setTimeout( function() {
							WPmobDoManageStatus( WPmobCustom.upload_done_header, WPmobCustom.upload_done_icon_status, true, 'success' );		
							setTimeout( function() { jQuery( '#manage-icon-set-area li a:first' ).click(); }, 250 ); }, 750 
					);					
				} else if ( response == 'zip' ) {				
					WPmobDoManageStatus( WPmobCustom.upload_unzip_header, WPmobCustom.upload_unzip_status, false, 'success' );			
					
					WPmobAdminAjax( 'manage-unzip-set', {}, function( result ) {
						if ( result == 'done' ) {
							setTimeout( 
								function() {								
									WPmobDoManageStatus( WPmobCustom.upload_done_header, WPmobCustom.upload_done_set_status, true, 'success' );
									setTimeout( function() { jQuery('#bnc-form').submit(); }, 250 ); }, 500
							);	
						} else if ( result == 'create-readme' ) {
							setTimeout( 
								function() {								
									WPmobDoManageStatus( WPmobCustom.upload_done_header, WPmobCustom.upload_describe_set, true, 'success' );
					...
					setTimeout( function() {
							WPmobDoManageStatus( WPmobCustom.upload_processing_header, WPmobCustom.upload_processing_status, false, 'success' );	
	...
	// Handle the select box for advertising
	...
	// Handle the home page re-direct select option
	...
	// Handle the show switch link setting
	...
	// Handle the web-app enabled option
	...
	// Handle the Classic post icon type select option	
	var wpmobPostIconType = jQuery( '#classic_icon_type' );
	if ( wpmobPostIconType.length ) {
		wpmobPostIconType.change( function() {
			var selectedValue = wpmobPostIconType.val();		
			if ( selectedValue == 'none' || selectedValue == 'calendar' ) {
				jQuery( '#setting_classic_thumbs_on_single' ).hide();
				jQuery( '#setting_classic_thumbs_on_pages' ).hide();	
			} else {
			...
			if ( selectedValue == 'thumbnails' ) {
				...
				jQuery( '#setting_post_thumbnails_new_image_size' ).hide();			
				jQuery( '#setting_regenerate-copytext-info' ).hide();			
			}
		});	
	}
	
	// Handle the calendar icon Appearance post icon type select option	
	var wpmobCalColor = jQuery( '#classic_icon_type' );
	if ( wpmobCalColor.length ) {
		wpmobCalColor.change( function() {
			var selectedValue = wpmobCalColor.val();		
	...
	if ( wpmobCalCustomColor.length ) {
		wpmobCalCustomColor.change( function() {
			var selectedParent = jQuery( '#classic_icon_type' ).val();	
			var selectedValue = wpmobCalCustomColor.val();		
			if ( selectedValue == 'cal-custom' && selectedParent == 'calendar' ) {
				...
				if ( postTags ) {
					...
					if ( postDesc ) {
						if ( postDesc ) {
							...
							// Ajax routine
							WPmobAdminAjax( 'support-posting', ajaxParams, function( result ) {				
								if ( result == "ok" ) {
						...
						alert( WPmobCustom.forum_topic_text );	
					}	
				} else {
	...
	if ( classicIconType.length ) {	
		classicIconType.change( function() {
			var selectItem = jQuery( this );
			
			if ( selectItem.val() == 'custom_thumbs' ) {
				jQuery( '#setting_classic_custom_field_thumbnail_name' ).fadeIn();	
			} else {
	...
	if ( classicCalCustom.length ) {	
		classicCalCustom.change( function() {
			var selectItem = jQuery( this );
			var selectedParent = jQuery( '#classic_icon_type' ).val();	
			
			if ( selectItem.val() == 'cal-custom' && selectedParent == 'calendar' ) {
				jQuery( '#setting_classic_custom_cal_icon_color' ).fadeIn();	
			} else {
				jQuery( '#setting_classic_custom_cal_icon_color' ).hide();
			}			
...
function WPmobHandleBackupRestore() {
	...
	if ( backupRestoreSelect.length ) {
		if ( backupRestoreSelect.val() == 'backup' ) {
			...
			jQuery( '#setting_backup' ).show();	
			
			WPmobHandleBackupClipboard();
						
		} else if ( backupRestoreSelect.val() == 'restore' ) {
			jQuery( '#setting_backup' ).hide();
			jQuery( '#setting_import' ).show();	
		}	
	}	
	
	backupRestoreSelect.unbind( 'change' ).change( function() {
		WPmobHandleBackupRestore();
	});
}
...
function WPmobTooltipSetup() {
	...
	// Admin Tooltips	
	jQuery( 'a.wpmob-tooltip' ).tooltip( { effect: 'fade', position: 'top right', offset: [-12, -10], tip: '#wpmob-tooltip' });	
	
	// Admin Tooltips for items on the far right of the page
	jQuery( 'a.wpmob-tooltip-left' ).tooltip( { effect: 'fade', position: 'top left', offset: [-10, 7], tip: '#wpmob-tooltip-left' });
	
	// Admin Tooltips for custom theme paths
...
function WPmobSetupThemeSettings() {	
	// Active Theme Settings Link
	jQuery( 'a.theme-settings' ).unbind( 'click' ).unbind( 'click' ).click( function() {
		jQuery( 'a#pane-3' ).click();
		return false;
	});	
}

function WPmobSetupActivateThemes() {	
	// Activate Theme
	jQuery( 'a.activate-theme' ).unbind( 'click' ).unbind( 'click ').click( function() {
		jQuery( '#pane-content-pane-2 .right-area' ).animate( { opacity: 0.45 } );	
		jQuery( '#ajax-saving' ).fadeIn( 350 ); 
...
function WPmobSetupCopyThemes() {
	...
	jQuery( 'a.copy-theme' ).unbind( 'click' ).unbind( 'click' ).click( function() {
		...
		jQuery( '#pane-content-pane-2 .right-area' ).animate( { opacity: 0.5 } );
		var themeLocation = jQuery( this ).parent().find( 'input.theme-location' ).attr( 'value' );
		var themeName = jQuery( this ).parent().find( 'input.theme-name' ).attr( 'value' );
...
function wpmobLoadSupportPosts() {	
	var wpmobboardSupport = jQuery( '#support-threads-box-ajax' );
...
function wpmobSetupTabSwitching() {
	...
	if ( adminTabSwitchLinks.length ) {
		adminTabSwitchLinks.unbind( 'click' ).click( function() {
			...
			setTimeout( function() {				
				jQuery( 'a.' + targetTabId ).click();
				jQuery( 'a#' + targetTabSection ).click();		
						
				jQuery( '#wpmob-tabbed-area' ).animate( { opacity: 1.0 }, 500 );
			},
			600 );
...
function WPmobDoManageStatus( header, status, all_done, class_to_add ) {
	setTimeout( 
		function() {
			...
			if ( all_done ) {
				jQuery( '#manage-spinner' ).hide();	
				jQuery( '#manage-set-upload-name' ).hide();				
			} else {
...
function WPmobSetupIconDragDrag() {
	...
	jQuery( '#wpmob-icon-menu div.icon-drop-target' ).droppable( {
		drop: function( event, ui ) {
			...
			droppedDiv.html( imageHtml );	
			droppedDiv.addClass( 'noborder' );			
...
function WPmobAdminAjax( actionName, actionParams, callback ) {	
	var ajaxData = {
		action: "wpmob_ajax",
...
function WPmobSetupLicenseArea() {		
	jQuery( 'a.wpmob-remove-license' ).unbind('click').click( function() {
...
//	jQuery( '#bnc' ).append( '<div id="wpmob-saving"></div>' );
	jQuery( '#ajax-loading' ).fadeIn(300);
}

function WPmobAjaxOff() {
//	jQuery( 'body' ).remove( '<div id="wpmob-saving"></div>' );
...
// Shows and area when a checkbox is enabled
...
function WPmobHandleBackupClipboard() {	
	if ( wpmobClip == 0 && jQuery( '#copy-text-button' ).is(':visible') ) {
